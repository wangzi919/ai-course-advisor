...
# -*- coding: utf-8 -*-
"""
此腳本會執行 OAAFormsScraper 來更新教務處相關文件表單資料。
...
# --- 路徑設定 ---
try:
    from scrapers.forms.oaa_forms import OAAFormsScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 OAAFormsScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新教務處相關文件表單資料...")
    try:
        scraper = OAAFormsScraper()
        scraper.force_update()
        logging.info("教務處相關文件表單資料更新成功。")
        sys.exit(0)
