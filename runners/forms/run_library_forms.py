...
# -*- coding: utf-8 -*-
"""
此腳本會執行 LibraryFormsScraper 來更新圖書館表單資料。
...
# --- 路徑設定 ---
try:
    from scrapers.forms.library_forms import LibraryFormsScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 LibraryFormsScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新圖書館表單資料...")
    try:
        scraper = LibraryFormsScraper()
        scraper.force_update()
        logging.info("圖書館表單資料更新成功。")
        sys.exit(0)
