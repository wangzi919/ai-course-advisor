...
# -*- coding: utf-8 -*-
"""
此腳本會執行 LibraryInstructionScraper 來更新圖書館利用指導課程資料。
...
# --- 路徑設定 ---
try:
    from scrapers.activities.library_instruction_service import LibraryInstructionScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 LibraryInstructionScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新圖書館利用指導課程...")
    try:
        scraper = LibraryInstructionScraper()
        scraper.force_update()
        logging.info("圖書館利用指導課程更新成功。")
        sys.exit(0)
