...
# -*- coding: utf-8 -*-
"""
此腳本會執行 EMIWorkshopsScraper 來更新 EMI 教學資源中心工作坊資料。
...
# --- 路徑設定 ---
try:
    from scrapers.activities.emi_workshops_scraper import EMIWorkshopsScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 EMIWorkshopsScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新 EMI 工作坊資料...")
    try:
        scraper = EMIWorkshopsScraper()
        scraper.force_update()
        logging.info("EMI 工作坊資料更新成功。")
        sys.exit(0)
