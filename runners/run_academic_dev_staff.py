...
# -*- coding: utf-8 -*-
"""
此腳本會執行 AcademicDevStaffScraper 來更新研發處學術發展組教職員資料。
...
# --- 路徑設定 ---
try:
    from scrapers.academic_dev_staff import AcademicDevStaffScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 AcademicDevStaffScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新研發處學術發展組教職員資料...")
    try:
        scraper = AcademicDevStaffScraper()
        scraper.force_update()
        logging.info("研發處學術發展組教職員資料更新成功。")
        sys.exit(0)
