...
# -*- coding: utf-8 -*-
"""
此腳本會執行 SchoolActivitiesNewsScraper 來更新學校活動新聞資料。
...
# --- 路徑設定 ---
try:
    from scrapers.news.school_news import SchoolActivitiesNewsScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 SchoolActivitiesNewsScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新學校活動新聞...")
    try:
        # 初始化爬蟲
        scraper = SchoolActivitiesNewsScraper()

        # 執行爬蟲以抓取、解析並儲存資料。
        # force_update() 會強制重新抓取，適合排程任務。
        scraper.force_update()

        logging.info("學校活動新聞更新成功。")
        sys.exit(0)
