...
# -*- coding: utf-8 -*-
"""
此腳本會執行 RDDirectorStaffScraper 來更新研發處研發長辦公室教職員資料。
...
# --- 路徑設定 ---
try:
    from scrapers.rd_director_staff import RDDirectorStaffScraper
except ImportError:
    ...
    try:
        ...
        logging.error(f"無法匯入 RDDirectorStaffScraper: {e}")
        sys.exit(1)
...
def main():
    ...
    主函式，用於初始化並執行爬蟲。
    """
    logging.info("開始更新研發處研發長辦公室教職員資料...")
    try:
        scraper = RDDirectorStaffScraper()
        scraper.force_update()
        logging.info("研發處研發長辦公室教職員資料更新成功。")
        sys.exit(0)
