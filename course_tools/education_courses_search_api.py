#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FastMCP tool that provides education program (教育學程) course search functionality for NCHU courses across all semesters."""
...
class EducationCourseSearcher:
    """教育學程課程搜尋器 - 支援多學期教育學程課程搜尋"""
    ...
    def __init__(self, data_dir: str = "data/courses/all_education_courses_syllabi"):
        """
        初始化教育學程課程搜尋器

        Args:
            data_dir: 教育學程課程資料目錄路徑
        """
        ...
        self.loaded_data: Dict[str, List[Dict]] = {}  # key: semester
    ...
    def _scan_available_semesters(self):
        ...
        for f in self.data_dir.glob("cross_*.json"):
            # 從 cross_1142.json 提取 1142
            semester = f.stem.replace("cross_", "")
            if len(semester) == 4 and semester.isdigit():
                semesters_set.add(semester)

        # 學期排序（最新的在前）
        self.available_semesters = sorted(list(semesters_set), reverse=True)

        if self.available_semesters:
            self.current_semester = self.available_semesters[0]
        else:
            raise RuntimeError(f"資料目錄中沒有找到教育學程課程資料: {self.data_dir}")
    ...
    def load_semester(self, semester: str) -> List[Dict]:
        ...
        Returns:
            該學期的教育學程課程列表
        """
        if semester in self.loaded_data:
            return self.loaded_data[semester]

        if semester not in self.available_semesters:
            available = self.available_semesters[:5]
            raise ValueError(
                f"學期 {semester} 不存在，可用學期: {', '.join(available)}...")

        json_path = self.data_dir / f"cross_{semester}.json"
            ...
            'semesters': [
                {
                    ...
                    'loaded': sem in self.loaded_data
                }
    ...
    def search_by_program(self, program_name: str, limit: int = 50, semester: Optional[str] = None):
        """按學程名稱搜尋課程"""
    ...
    def get_all_programs(self, semester: Optional[str] = None):
        """
        取得所有學程列表及課程數量
    ...
    def get_program_courses(self, program_name: str, semester: Optional[str] = None):
        """
        取得特定學程的所有課程
    ...
    def get_course_detail(self, course_id: str, include_syllabus: bool = True, semester: Optional[str] = None):
        ...
        return {
            'found': False,
            'message': f'找不到選課號碼為 {course_id} 的教育學程課程',
            'metadata': self._get_metadata(sem)
        }
...
# 初始化全域搜尋器
searcher = EducationCourseSearcher()

mcp = FastMCP("nchu_education_course_search")


@mcp.tool()
def nchu_education_search_by_keyword(
    keyword: str,
    limit: int = 10,
    ...
    """Search for education program (教育學程) courses in NCHU course database.
...
@mcp.tool()
def nchu_education_search_across_semesters(
    keyword: str,
    limit_per_semester: int = 10,
    search_fields: str | None = None,
    semesters: str | None = None
) -> str:
    """Search for education program courses across multiple semesters. Useful for finding historical course offerings.
    ...
    try:
        ...
        return json.dumps({
            'error': f'跨學期搜尋教育學程課程時發生錯誤: {str(e)}',
            'results': {},
            'total': 0
        }, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_get_available_semesters() -> str:
    """Get all available semesters in the education program course database.
...
@mcp.tool()
def nchu_education_search_by_program(
    program_name: str,
    limit: int = 50,
    semester: str | None = None
) -> str:
    """Search education program courses by program name (學程名稱).

    Args:
        program_name: Program name to search for (e.g., 中等學程, 小學學程)
    ...
    """
    try:
        results = searcher.search_by_program(program_name, limit, semester)
        return json.dumps(results, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_get_all_programs(semester: str | None = None) -> str:
    ...
    """
    try:
        results = searcher.get_all_programs(semester)
        return json.dumps(results, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_get_program_courses(
    program_name: str,
    semester: str | None = None
) -> str:
    ...
    Args:
        program_name: Program name (e.g., 中等學程)
        semester: Semester code. If not specified, uses the latest semester.
    ...
    """
    try:
        results = searcher.get_program_courses(program_name, semester)
        return json.dumps(results, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_search_by_department(
    department: str,
    limit: int = 20,
    semester: str | None = None
) -> str:
    """Search education program courses OFFERED by a specific department.

    Args:
        department: Department name to search for (e.g., 師資培育中心)
    ...
    try:
        ...
        return json.dumps({
            'error': f'按系所搜尋教育學程課程時發生錯誤: {str(e)}',
            'results': [],
            'total': 0
        }, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_search_by_teacher(
    teacher_name: str,
    limit: int = 20,
    semester: str | None = None
) -> str:
    """Search education program courses by teacher name.
    ...
    try:
        ...
        return json.dumps({
            'error': f'按授課教師搜尋教育學程課程時發生錯誤: {str(e)}',
            'results': [],
            'total': 0
        }, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_search_by_time(
    weekday: str,
    period: str | None = None,
    limit: int = 20,
    semester: str | None = None
) -> str:
    """Search education program courses by class time.
    ...
    try:
        ...
        return json.dumps({
            'error': f'搜尋教育學程課程上課時間時發生錯誤: {str(e)}',
            'results': [],
            'total': 0
        }, ensure_ascii=False, indent=2)
...
@mcp.tool()
def nchu_education_get_stats(semester: str | None = None) -> str:
    """Get statistics about the education program course database.
...
@mcp.tool()
def nchu_education_get_detail(
    course_id: str,
    include_syllabus: bool = True,
    semester: str | None = None
) -> str:
    """Get detailed information about a specific education program course, including full syllabus.
    ...
    try:
        ...
        return json.dumps({
            'error': f'獲取教育學程課程詳細資訊時發生錯誤: {str(e)}',
            'message': '無法獲取教育學程課程詳細資訊'
        }, ensure_ascii=False, indent=2)


@mcp.tool()
def nchu_education_get_all_teachers(semester: str | None = None) -> str:
    """Get all teachers and their education program course statistics.
...
@mcp.tool()
def nchu_education_get_all_departments(semester: str | None = None) -> str:
    """Get all departments and their education program course counts.
...
@mcp.tool()
def nchu_education_time_format_help() -> str:
    ...
    help_text = {
        ...
        "星期對照": EducationCourseSearcher.WEEKDAY_MAP,
        "節次對照": EducationCourseSearcher.TIME_SLOTS,
        "範例": {
...
@mcp.tool()
def nchu_education_search_syllabus(
    keyword: str,
    limit: int = 10,
    include_syllabus_in_result: bool = False,
    semester: str | None = None
) -> str:
    """Search education program courses by syllabus content.
...
@mcp.tool()
def nchu_education_get_weekly_content(
    course_id: str,
    semester: str | None = None
) -> str:
    """Get weekly course content for a specific education program course.
